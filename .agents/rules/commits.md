...
4. **Commits Atômicos (Separação Granular)**: ANTES de sugerir o commit, você **deve** realizar a separação do que foi efetuado o mais granular possível. Nunca agrupe alterações de contextos diferentes no mesmo commit (ex: "várias alterações" ou "refatoração + nova feature"). Separe os arquivos (`git add`) logicamente de acordo com o contexto exato do que foi feito.
5. **PROIBIÇÃO DE COMMIT DIRETO**: Ao terminar uma atividade, você **NÃO DEVE** executar o comando `git commit` diretamente via terminal. Em vez disso, você deve **retornar uma sugestão** de um ou mais commits atômicos para que o próprio usuário revise e suba os arquivos. Se houver mais de um contexto alterado (múltiplos commits sugeridos), forneça o comando exato de cópia e cola para cada um, especificando os arquivos (ex: `git add src/arquivo1.ts && git commit -m "..."`) para facilitar a vida do usuário.
